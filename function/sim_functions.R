...
###############################
getOneLayerProp <- function(pattern_gp_label,ipt,ct.select,noise.p,ntotal = 10){
  
  nobs = sum(pattern_gp_label == ipt) ## total number of spot in this layer
  numCT = sample(2:length(ct.select),1) ## total number of cell types in each layer: main cell type + colocalized cell types
  ...
  mix_spot = sample(rownames(prop_this_layer),round(nobs * noise.p)) ## noisy spots
  fix_spot = setdiff(rownames(prop_this_layer),mix_spot)
  if(length(mix_spot) > 0){
    prop_this_layer[mix_spot,] = rdirichlet(length(mix_spot),rep(1,numCT))[,sample(1:numCT,replace = F)]
  }
...
getProportion <- function(pattern_gp_label,ct.select,noise.p = noise_p,ntotal = 10){
  ...
  pattern1 = getOneLayerProp(pattern_gp_label,1,ct.select,noise.p = noise.p,ntotal=10)
  pattern2 = getOneLayerProp(pattern_gp_label,2,ct.select,noise.p = noise.p,ntotal=10)
  pattern3 = getOneLayerProp(pattern_gp_label,3,ct.select,noise.p = noise.p,ntotal=10)
...
##############################
getSpotMix <- function(eset.sub.split1,pattern_gp_label,ct.select,noise_p = noise_p,ntotal = 10){
  ...
  out = getProportion(pattern_gp_label,ct.select,noise.p = noise_p,ntotal = 10)
